...
public class DemoSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		// jdbcAuthentication -> tells spring to use jdbc auth
	...
	protected void configure(HttpSecurity http) throws Exception { // http secuirty, we use for spring secutiry config
		...
		http.authorizeRequests()
				...
				.antMatchers("/").hasRole("EMPLOYEE").antMatchers("/leaders/**").hasRole("MANAGER")// /** means all
																									// subdiretives
				...
				.loginProcessingUrl("/authenticateTheUser") // spring security for check user, this is automagic for
															// spring
				.permitAll() // as long we give the data or fields -> user pass
				// permitAll() -> allow everyone to see the login page
				.and().logout().permitAll() // enable log out support
				.and().exceptionHandling().accessDeniedPage("/access-denied"); // redirect to this pages when role
																				// doestn have access
	}
